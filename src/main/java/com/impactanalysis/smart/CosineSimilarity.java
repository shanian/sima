...
public class CosineSimilarity {
    ...
    public void getCosineSimilarity(String jiraStoryNum, List<String> jiraStoryNumber,  List<double[]> tfidfDocsVector) {

		int jiraStoryNumIndex = 0;
		for (int i=0;i<jiraStoryNumber.size();i++){
			if (jiraStoryNumber.get(i).equalsIgnoreCase(jiraStoryNum)){
				jiraStoryNumIndex=i;
				break;
			}
		...
		for (int i = 0; i < tfidfDocsVector.size(); i++) {
				...
				System.out.println("between " + jiraStoryNumber.get(jiraStoryNumIndex) + " and " + jiraStoryNumber.get(i) + "  =  "
						+ cosineSimilarity					(
								...
								tfidfDocsVector.get(jiraStoryNumIndex)
								)
						);
			}
